// Please allow same site scripting to use the script
...
Linkedin = {
    ...
    init: function (data, config) {
        console.info("INFO: script initialized on the page we now let the computer handel your connections :)");
    ...
    compile: function (data, config) {
        ...
        if (!data.pageButtons || data.pageButtons.length === 0) {
            console.warn("ERROR: no connect buttons found on page! I Cant do everything REEEEE");
    ...
    sendInvites: function (data, config) {
        ...
        if (config.maxRequests == 0){
            console.info("INFO: max requests reached for the script run! dont do more if ya dont wanna get banned");
            this.complete(config);
        } else {
